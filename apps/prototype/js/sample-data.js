...
export function generateSampleCSV(months = 6) {
  const rows = [];
  const now = new Date();
  ...
  for (let d = new Date(startDate); d <= now; d.setDate(d.getDate() + 1)) {
    ...
    for (const merchant of MERCHANTS) {
      ...
      if (merchant.monthly) {
        // Monthly items: appear around day 5-10 (salary), or matching their usual day
        ...
        else if (merchant.type === 'expense' && dayOfMonth === rand(8, 12) && d.getDate() >= 8 && d.getDate() <= 12) shouldAdd = true;
        // For subscriptions, pick a fixed day
        ...
        shouldAdd = Math.random() < merchant.freq / 30; // daily probability
      }
