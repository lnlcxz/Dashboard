...
export function renderLineChart(containerId: string, byMonth: Record<string, MonthBucket>): void {
  ...
  chart.setOption({
    tooltip: {
      ...
      borderColor: 'rgba(255,255,255,0.08)', textStyle: { color: '#fff', fontSize: 13 },
    ...
    xAxis: { type: 'category', data: labels, boundaryGap: false, axisLine: { lineStyle: { color: CHART_THEME.lineColor } }, axisLabel: { color: CHART_THEME.textColor, fontSize: 12 } },
    ...
    series: [
      { name: 'Receitas', type: 'line', smooth: true, symbol: 'circle', symbolSize: 6, data: months.map(m => byMonth[m]!.income), lineStyle: { width: 3, color: '#10b981' }, itemStyle: { color: '#10b981' }, areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(16,185,129,0.25)' }, { offset: 1, color: 'rgba(16,185,129,0)' }]) } },
      { name: 'Despesas', type: 'line', smooth: true, symbol: 'circle', symbolSize: 6, data: months.map(m => byMonth[m]!.expense), lineStyle: { width: 3, color: '#f43f5e' }, itemStyle: { color: '#f43f5e' }, areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(244,63,94,0.20)' }, { offset: 1, color: 'rgba(244,63,94,0)' }]) } },
    ],
  });
}
...
export function renderDoughnutChart(containerId: string, byCat: Record<string, number>): void {
  ...
  chart.setOption({
    tooltip: { trigger: 'item', backgroundColor: CHART_THEME.tooltipBg, borderColor: 'rgba(255,255,255,0.08)', textStyle: { color: '#fff', fontSize: 13 }, formatter: (p: { marker: string; name: string; value: number; percent: number }) => `${p.marker} ${p.name}<br/>${formatBRL(p.value)} (${p.percent.toFixed(1)}%)` },
    series: [{ type: 'pie', radius: ['48%', '75%'], center: ['50%', '50%'], avoidLabelOverlap: true, padAngle: 2, itemStyle: { borderRadius: 6, borderColor: '#0f1420', borderWidth: 2 }, label: { show: true, color: CHART_THEME.textColor, fontSize: 11, formatter: '{b}\n{d}%', lineHeight: 16 }, labelLine: { lineStyle: { color: 'rgba(255,255,255,0.15)' } }, emphasis: { label: { fontSize: 14, fontWeight: 'bold' }, itemStyle: { shadowBlur: 20, shadowColor: 'rgba(0,0,0,0.4)' } }, data }],
  });
}
