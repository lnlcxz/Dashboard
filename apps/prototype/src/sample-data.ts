// ============================================
// FINDASH — Sample CSV Data Generator
// ============================================
...
export function generateSampleCSV(months = 6): string {
  const rows: Array<{ date: Date; description: string; value: number; type: string }> = [];
  const now = new Date();
  const startDate = new Date(now.getFullYear(), now.getMonth() - months, 1);

  for (const d = new Date(startDate); d <= now; d.setDate(d.getDate() + 1)) {
    ...
    for (const merchant of MERCHANTS) {
      ...
      if (merchant.monthly) {
        ...
        else if (merchant.type === 'expense' && dayOfMonth >= 8 && dayOfMonth <= 12 && dayOfMonth === rand(8, 12)) shouldAdd = true;
      } else if (merchant.freq) {
        shouldAdd = Math.random() < merchant.freq / 30;
      }
      if (shouldAdd) {
        const amount = rand(merchant.min, merchant.max);
        rows.push({ date: new Date(d), description: merchant.name, value: merchant.type === 'expense' ? -amount : amount, type: merchant.type === 'income' ? 'Receita' : 'Despesa' });
      }
    }
  }
...
export function downloadSampleCSV(): void {
  ...
  a.href = url; a.download = 'findash-exemplo-6meses.csv';
