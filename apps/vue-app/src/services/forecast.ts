import type { Forecast, RecurringItem, Transaction } from '@/types';
...
function getMonthlyTotals(transactions: Transaction[], months = 6) {
  const now = new Date();
  const result: { key: string; income: number; expense: number; net: number; date: Date }[] = [];
...
export function generateForecast(transactions: Transaction[], recurringItems: RecurringItem[] = [], horizon = 30): Forecast {
